...
Production approach: Use save_results=True and read output files.
Source: https://huggingface.co/deepseek-ai/DeepSeek-OCR/discussions/30
"""
...
def handler(job):
    ...
    Uses file-based output capture (production approach).
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Gundam mode with file output (production approach)
            # model.infer() returns None - output is written to files
            model.infer(
                tokenizer,
                prompt=prompt,
                ...
                save_results=True,   # Write output to files
                test_compress=False, # Standard inference
            )

            # Read the output file (model writes to result.mmd)
            markdown = ""

            # Try result.mmd first (standard output)
            result_file = os.path.join(temp_dir, "result.mmd")
            if os.path.exists(result_file):
                with open(result_file, 'r', encoding='utf-8') as f:
                    markdown = f.read()
            else:
                # Fallback: look for any .mmd or .txt file
                for filename in os.listdir(temp_dir):
                    if filename.endswith(('.mmd', '.txt')) and filename != 'input.png':
            ...
            if not markdown:
                return {
                    "status": "error",
                    "error": "No output file generated by model.infer()",
